...
const CalendarGrid: React.FC<CalendarGridProps> = ({
  ...
  const getTaskPosition = (task: Task) => {
    ...
    const startPosition =
      (taskStartHour - startHour + taskStartMinute / 60) * 64;
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    const newStartHour = startHour + Math.floor(totalMinutesFromStartHour / 60);
    const newStartMinute = totalMinutesFromStartHour % 60;
    ...
    const maxTimeBoundary = setMinutes(
      ...
      0
    ); // Max start time for a task to fit
    ...
    if (newEndTime > addHours(maxTimeBoundary, 1)) {
      // allow task to end at endHour:00
    ...
    const collision = tasks.some((task) => {
      ...
      const existingTaskStart =
        task.startTime instanceof Date
          ? task.startTime
          : new Date(task.startTime);
      const existingTaskEnd =
        task.endTime instanceof Date ? task.endTime : new Date(task.endTime);

      // Check for overlap: (StartA < EndB) and (EndA > StartB)
      return (
        newStartTime < existingTaskEnd &&
        addMilliseconds(newStartTime, taskDuration) > existingTaskStart
      );
    });
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const startHourDecimal =
      roundedStartTime.getHours() -
      startHour +
      roundedStartTime.getMinutes() / 60;
    const startY = startHourDecimal * 60;
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Update visual range
    const dragStartY =
      (dragStart.getHours() - startHour + dragStart.getMinutes() / 60) * 64;
